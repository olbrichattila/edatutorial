...
func main() {
	...
	if err := eventManager.Consume(topic, consumer, handleSendInvoiceEmail(logger, invoiceRepository, actionStoreRepository)); err != nil {
		logger.Error(fmt.Sprintf("send invoice email consumer error: %v", err))
	}
}

func handleSendInvoiceEmail(
	logger loggerContracts.Logger,
	invoiceRepository invoiceContracts.InvoiceRepository,
	...
	return func(evt contracts.EventManager, msg []byte) error {
		...
		if err != nil {
			logger.ErrorWithAction(invoiceCreatedAction.MetaData, "cannot fetch invoice: "+err.Error())
			return err
		}
		...
		if err != nil {
			logger.ErrorWithAction(invoiceCreatedAction.MetaData, "cannot fetch invoice HTML: "+err.Error())
			return err
		}
		...
		if err != nil {
			logger.ErrorWithAction(invoiceCreatedAction.MetaData, "send email error: "+err.Error())
			return err
		}
